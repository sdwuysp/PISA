...
class BaseModel():
    ...
    def resume_training(self, resume_state):
        ...
        Modified to support CosineAnnealingRestartLR without crashing.
        """
        ...
        for i, s in enumerate(resume_schedulers):
            self.schedulers[i].last_epoch = resume_state['iter']
            
            current_optim_key = 'optim_g' if i == 0 else 'optim_d'
            if current_optim_key in self.opt['train']:
                new_lr_cfg = self.opt['train'][current_optim_key].get('lr')
                
                if new_lr_cfg is not None:
                    new_lr = float(new_lr_cfg)
                    self.schedulers[i].base_lrs = [new_lr for _ in self.schedulers[i].base_lrs]
                    ...
                    print(f"  [Resume] 🔄 Scheduler Reset & LR Updated: {current_optim_key} -> {new_lr} (Iter: {resume_state['iter']})")
