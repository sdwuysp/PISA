...
class TVLoss(nn.Module):
    ...
    def forward(self, x, gt=None):
        # x: current generated image [B, C, H, W]
        # gt: ground truth (unused in TV loss, but kept for interface consistency)
        ...
        loss = self.loss_weight * 2 * (h_tv / count_h + w_tv / count_w) / batch_size
        return loss

    def _tensor_size(self, t):
        return t.size()[1] * t.size()[2] * t.size()[3]
    
# [新增] 放在 basicsr/losses/losses.py 末尾
@LOSS_REGISTRY.register()
class GradientWeightedTVLoss(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(GradientWeightedTVLoss, self).__init__()
        self.loss_weight = loss_weight
        self.reduction = reduction

    def forward(self, pred, weight=None):
        y_diff = torch.abs(pred[:, :, :-1, :] - pred[:, :, 1:, :])
        x_diff = torch.abs(pred[:, :, :, :-1] - pred[:, :, :, 1:])
        
        y_diff = F.pad(y_diff, (0, 0, 0, 1))
        x_diff = F.pad(x_diff, (0, 1, 0, 0))
        
        tv_loss = y_diff + x_diff

        # 核心修改：如果传入了 weight (掩码)，就进行空间加权
        if weight is not None:
            tv_loss = tv_loss * weight

        if self.reduction == 'mean':
            return self.loss_weight * torch.mean(tv_loss)
        elif self.reduction == 'sum':
            return self.loss_weight * torch.sum(tv_loss)
        else:
            return self.loss_weight * tv_loss